...
fog_B_fix_server = zeros(num_a,1);


real_time = zeros(num_a,1);
big_data = zeros(num_a,1);
...
%         unitBandCost = 0.05*ones(M,K); % unit cost of different connectivity options (0.05 $/GB)
        unitBandCost = rand(M,K).*0.15+0.1;
...
%     real_time(a_index,1) = sum(sum(migDataFromDC))/Iters;
%     big_data(a_index,1) = sum(sum(fog_B))/Iters;
    fog_B_fix_server(a_index,1) = sum(sum(fog_B))/Iters;
