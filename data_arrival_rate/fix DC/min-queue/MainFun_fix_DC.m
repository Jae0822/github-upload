...
%power_idle = 600; %unit: W
slope_m = 30.6/8; %unit: Joule per Gigabit to GB/s
fog_capacity = (10)*1.8/8; %unit: Gbps to GB/s
...
%speed_option = [2.0 2.2 2.4 2.6 2.8];   %in GHz
...
%% -----Algorithms-----------------------------------------
% Define Non-negative weight of penalty function
% a_options=[1 3 9 14 18 20];
% a_options=[36 40 45 50 55 60];
a_options=[1 3 9 14 18 20 35 40 45 50 55 60];
...
for a_variable = 1:num_a
    a_index = a_variable;
    mu = a_options(a_variable);
    sigma = mu./2;
    a = normrnd(mu,sigma,[Iters,M]);
%     a = (variance*randn(Iters,M) + a_options(a_variable));

    %指定reducer
    ind = round(rand(1,1)*3)+1;  %指定Index,即为指定reducer。
    ss = zeros(1,K);
    ss(1,ind) = 1;
    s = repmat(ss,Iters,1);
%     s = zeros(Iters,K);     % vector for aggeragator DCs
    ...
    %%-------------generating fog GW and DC points----------
    %画图脚本里面的bandWidth1，bandWidthR，bandWidth01
    %即是通过改变diatance_logic的初始值和更改值实现的。
    fog_location = zeros(M,2);
    DC_location = zeros(K,2);
    distance = zeros(M,K);
    distance_logic1 = zeros(M,K);
    distance_logic2 = R.*ones(M,K);
    for ff = 1:M
        fog_location(ff,:) = 2*rand(1,2)-1;
        rr = fog_location(ff,:);
        while sum(rr.^2) > 1
            rr = 2*rand(1,2)-1;
        end
    ...
    for ff = 1:M
        for dd = 1:K
            distance(ff,dd) = sqrt(sum((fog_location(ff,:)-DC_location(dd,:)).^2));
        end
    end
    %----------reset distance_logic-------------------
    for ff = 1:M
        [m0,index] = min(distance(ff,:));
        distance_logic1(ff,index) = 1;
        distance_logic2(ff,index) = 1;
    end
    ...
    for iter = 1:Iters
        waitbar((a_index*iter) / (num_a*Iters));
...
%         variance2=100;  
        u2=100;%论文中的参数
        variance2=20;
        bandWidth = (2)*tau*(variance2*randn(M,K) + u2)/Mbps2GBps;
        bandWidth = bandWidth.*distance_logic1;
        
%         unitBandCost = 0.05*ones(M,K); % unit cost of different connectivity options (0.05 $/GB)
        unitBandCost = rand(M,K).*0.15+0.3;
        
%         unitBandCost = unitBandCost.*distance_logic2;
        
        unitMigrationCost = 0.05*ones(K,K);    %unit cost(0.05 $/GB)
%         unitMigrationCost = rand(K,K).*0.15+0.15; 
...
%             z(i,:) = 0;
            power_fog(iter,i) = ((3)*slope_m*fog_B(iter,i)/tau+power_idle)/w2mw;
...
%         [index, s(iter,:)] = SelectDC(Q(iter,:),m_tmp,mm_tmp,V);  % DC selection function
        ...
        for j = 1:K
            ...
            %             r(iter,subscript) = mu2(iter,j) + m(j,subscript);
            %             mm(j,subscript) = unitMigrationCost(j,subscript)*m(j,subscript);
...
%set(hLine2,'LineStyle','-','Marker','^','Color','r');
set(hLine2,'LineStyle','-','Marker','s','Color','r');
%set(hLine2,'LineStyle','-','Marker','*','Color','r');
%set(hLine2,'LineStyle','-','Marker','d','Color','r');
set(hAx(1),'YColor','b');
set(hAx(2),'YColor','r');
grid
